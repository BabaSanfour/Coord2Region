...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    output_path = Path(args.output) if args.output else template_path.parent / output_name
    ...
    if output_path.exists():
        ...
        if not args.force and not args.non_interactive:
            reply = input(
                f"{output_path} already exists. Overwrite values while preserving existing defaults? [y/N]: "
            ).strip()

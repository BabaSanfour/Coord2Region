...
def infer_hemisphere(coord: Sequence[float]) -> str:
    """Infer left/right/midline hemisphere from the MNI x coordinate."""
...
def run_reasoned_report(
    ...
    *,
    ...
    messages = build_reasoned_report_messages(
...
def build_region_image_request(
    ...
    *,
    ...
    spec = {
        "figure_goal": "Show the anatomical location of the given MNI coordinate on an MNI152-like template with overlays.",
    ...
    positive_prompt = (
        ...
        f"bold red spherical highlight (radius {sphere_radius_mm} mm) centered at MNI {coord_str}, "
        "thin white crosshair at the coordinate, "
        f'labels: "{primary_label} ({hemisphere})" near the highlight and "MNI {coord_str}" bottom-left, '
        "clean medical styling, high contrast, crisp lines, 1024x1024, subtle 'Illustrative' watermark in a corner."
    )
